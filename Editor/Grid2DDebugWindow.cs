...
namespace BatuhanYigit.Grid2D.Editor
{
    public abstract class AbstractGridDebuggerWindow<T> : EditorWindow
    {
        ...
        protected virtual void OnGUI()
        {
            ...
            if (newDebugState != _isEnabled)
            {
                ...
                {
                    var monoBehaviours = rootObj.GetComponentsInChildren<MonoBehaviour>(true);
                    foreach (var mb in monoBehaviours)
                    {
                        if (mb is IGridProvider<T> provider)
                        {
                            _selectedGridProviderObj = mb;
                            _gridProvider = provider;
